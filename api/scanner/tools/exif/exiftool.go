...
var (
	exiftoolPath  string
	exiftoolError error = tools.ErrInvalid
)

func initExiftool(ctx context.Context) error {
	path, err := exec.LookPath("exiftool")
	if err != nil {
		return fmt.Errorf("find exiftool binary error: %w", err)
	}

	voutput, err := exec.Command(path, "-ver").Output()
	if err != nil {
		return fmt.Errorf("run `exiftool -ver` error: %w", err)
	}

	version := string(bytes.Trim(voutput, " \t\n\r"))
	if version == "" {
		return fmt.Errorf("run `exiftool -ver` error: no output")
	}

	log.Info(ctx, "exiftool", "path", path, "version", version)
...
func init() {
	globalinit.Register("exiftool", initExiftool, nil)
}
...
func New() (*Exiftool, error) {
	if exiftoolError != nil {
		return nil, fmt.Errorf("exiftool: %w", exiftoolError)
	}
